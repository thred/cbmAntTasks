...
public class DefaultCbmAllocateSectorStrategy implements CbmAllocateSectorStrategy
{
    @Override
        ...
        throws CbmIOException
    {
        ...
        CbmSectorLocation location = optionalLocation.orElse(directoryLocation);
        CbmSectorInterleaves sectorInterleaves = disk.getSectorInterleaves();
        ...
        if (trackNr == directoryTrackNr)
        {
            ...
            {
                ++count;
                trackNr = directoryTrackNr - count;

                if (trackNr >= 1 && disk.getFreeSectorCountOfTrack(trackNr, true) > 0)
                {
                    break;
                }
                ...
                if (trackNr <= sectorMap.getMaxTrackNr() && disk.getFreeSectorCountOfTrack(trackNr, true) > 0)
                {
                    break;
                }

                if (count > sectorMap.getMaxTrackNr() - 1)
                {
                    throw new CbmIOException(Type.DISK_FULL);
                }
            }
        }
        else if (disk.getFreeSectorCountOfTrack(trackNr, true) == 0)
        {
            // Sequential seek
            int increment = trackNr < directoryTrackNr ? -1 : 1;
            ...
            while (true)
            {
                ...
                if (trackNr < 1 || trackNr > sectorMap.getMaxTrackNr())
                {
                    increment = -increment;
                    trackNr = directoryTrackNr;
                    sectorNr = 0;
            ...
            interleave = subsequent ? sectorInterleaves.getInterleave(trackNr) : 1;
            sectorNr = computeNextSector(sectorNr, sectorMap.getMaxSectorNrByTrackNr(trackNr), interleave);
            interleave = 1;
        }
